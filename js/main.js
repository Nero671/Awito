...
const modalAdd = document.querySelector('.modal__add'),
      ...
      modalFilterBtn = document.querySelector('.modal__file-btn'),
...
const textFileBtn = modalFilterBtn.textContent;
...
const closeModal = event => {
  ...
  if(target.closest('.modal__close') || target.classList.contains('modal') ||
      event.code === 'Escape') {
        ...
        modalFilterBtn.textContent = textFileBtn;
        checkForm();
      }
};
...
modalFileInput.addEventListener('change', event => { //загрузка картинок
  const target = event.target;
  ...
  reader.addEventListener('load', event => {
    if(infoPhoto.size < 2000000) {
      modalFilterBtn.textContent = infoPhoto.filename;
      infoPhoto.base64 = btoa(event.target.result);
      modalImageAdd.src = `data:image/jpeg;base64,${infoPhoto.base64}`;
    } else {
      modalFilterBtn.textContent = 'файл не должен первышать 20000кБ';
